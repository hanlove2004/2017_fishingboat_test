...
<body>
	...
	<script type="text/javascript">
		...
		function fn_openInfoDetail(obj) {
			var comSubmit = new ComSubmit();
			comSubmit.setUrl("<c:url value='/info/openInfoDetail.do' />");
			comSubmit.addParam("shipNo", obj.parent().find("#shipNo").val());
			comSubmit.submit();
		}
		
		function fn_selectInfoList(pageNo) {
			var comAjax = new ComAjax();
			comAjax.setUrl("<c:url value='/info/selectInfoList.do' />");
			comAjax.setCallback("fn_selectInfoListCallback");
			comAjax.addParam("PAGE_INDEX", pageNo);
			comAjax.addParam("PAGE_ROW", 10);
			comAjax.ajax();
		}
		
		function fn_searchInfoList() {
			var comAjax = new ComAjax();
			comAjax.setUrl("<c:url value='/info/selectInfoList.do' />");
			comAjax.setCallback("fn_selectInfoListCallback");
		...
		function fn_selectInfoListCallback(data) {
			...
			if (total == 0) {
				...
				$.each(data.list, function(key, value) {
					str += "<tr>" +
							...
							"<td>" + value.HANG_NAME + "</td>" +
							"<td>" + value.TON + " 톤</td>" +
							"<td>" + value.CODE_CTN_3 + "</td>" +
						"</tr>";
				});
				body.append(str);
