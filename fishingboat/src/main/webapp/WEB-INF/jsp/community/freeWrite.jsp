			...
			<form id="frm" name="frm">
				<table class="table table-striped table-bordered table-hover">
					...
					<tbody>
						<tr>
							<th scope="row">작성자</th>
							<td><c:out value="${mberInfo.NAM }" /></td>
						</tr>
						<tr>
							<th scope="row">제목</th>
							<td><input type="text" title="제목" id="title" name="title" class="form-control" style="width: 700px;" /></td>
						</tr>
						<tr>
							<th scope="row">내용</th>
							<td>
								<textarea rows="20" cols="100" title="내용" id="contentArea" name="contentArea"></textarea>
							</td>
						</tr>
					</tbody>
				</table>
		...
		function fn_insertFree() {
			if($("#title").val() == ""){
				...
				if(confirm("등록하시겠습니까?") == true){
					$.ajax({
						url : "<c:url value='/community/insertFree.do' />",
						type : "POST",
						data : $("#frm").serialize(),
						async : false,
						success : function(responseData, textStatus, jqXHR) {
							if (textStatus == "success") {
								alert("등록이 완료되었습니다. 목록 페이지로 이동합니다.");
								fn_openFreeList();
							} else {
