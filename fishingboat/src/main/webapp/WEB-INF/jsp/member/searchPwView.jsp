...
<body>
		...
		<div id="content" style="min-height: 470px;">
			<div id="login" style="margin: 80px;">
				<div class="well" style="text-align: center; vertical-align: middle; margin: auto; width: 500px; border-radius: 0.5em; background-color: #E5F1B5; box-shadow: 0 0 10px gray inset, 0 0 10px black;">
					<h1 style="font-family: myfont05;">비밀번호찾기</h1>
					<hr>
					<form id="searchform" method="post">
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control" id="nam" name="nam" placeholder="이름" onkeydown="if(event.keyCode == 13){$('#usid').focus();}">
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-sm-8">
								<input type="text" class="form-control" id="usid" name="usid" placeholder="아이디"  onkeydown="if(event.keyCode == 13){$('#email').focus();}">
							</div>
						</div>
						<div class="form-group">
							<label for="password" class="col-sm-4 control-label" style="font-size: 20px; text-align: right; vertical-align: middle;">
					...
					<br /><br />
					<button type="submit" id="searchbtn" class="btn btn-success" onclick="fn_selectUsrPwSearch();">찾기</button>
				</div>
			</div>
	...
	<script type="text/javascript">
		...
		function fn_selectUsrPwSearch(){
			$.ajax({
				url : "<c:url value='/member/selectUsrPwSearch.do' />",
				type : "POST",
				...
				success : function(data, status) {
					...
					if (data.result != undefined) {
						alert("입력하신 " + data.result.USR_EMAIL + " 로 비밀번호를 발송했습니다. 로그인 페이지로 이동합니다.");

						//페이지 이동
						var url = "<c:url value='/member/openLoginView.do' />"; 
