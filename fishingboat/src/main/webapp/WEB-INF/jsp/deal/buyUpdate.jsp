...
<body>
						...
						<tr>
							...
							<td>
								<input type="text" id="price" name="price" style="text-align: right;" maxlength="15" class="form-control"
									...
									value="<c:out value='${buyResult.HOPE_PRICE }' />" />
							</td>
							<td>
						...
						<tr>
							...
							<td>
								<input type="text" id="ton" name="ton" style="text-align: right;" maxlength="15" class="form-control" title="총톤수" placeholder="총톤수"
									onkeypress="isNumObj(this);" onkeydown="if(event.keyCode == 13){$('#price').focus();}"
									value="<c:out value='${buyResult.SHIP_TOT_TONS }' />"/>
							</td>
							<td colspan="2"></td>
						</tr>
						<tr>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			var type = "${buyResult.FS_MET_CODE}";
			var area = "${buyResult.REL_TRANS_CGG_CODE}";
			$("#type").val(type);
			$("#area").val(area);
		...
		function fn_updateBuy() {
			if(!$("#price").val()){
				...
				if(confirm("수정하시겠습니까?")){
					$.ajax({
						url : "<c:url value='/deal/updateBuy.do' />",
						type : "POST",
						data : $("#frm").serialize(),
						async : false,
