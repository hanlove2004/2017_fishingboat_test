...
<body>
	...
	<div class="container">
		...
		<div id="content" style="min-height: 470px;">
			...
			<div align="right" style="padding-right: 30px; padding-bottom: 10px; height: 50px;">
				<a href="#this" class="btn" id="write">어선매수 신청</a>
			</div>
	...
	<script type="text/javascript">
		...
		function fn_openBuyWrite() {
			var comSubmit = new ComSubmit();
			comSubmit.setUrl("<c:url value='/deal/openBuyWrite.do' />");
			comSubmit.submit();
		}
		
		function fn_openBuyDetail(obj) {
			var comSubmit = new ComSubmit();
			comSubmit.setUrl("<c:url value='/deal/openBuyDetail.do' />");
			comSubmit.addParam("seq", obj.parent().find("#seq").val());
			comSubmit.submit();
		}
		
		function fn_selectBuyList(pageNo) {
			var comAjax = new ComAjax();
			comAjax.setUrl("<c:url value='/deal/selectBuyList.do' />");
			comAjax.setCallback("fn_selectBuyListCallback");
			comAjax.addParam("PAGE_INDEX", pageNo);
			comAjax.addParam("PAGE_ROW", 10);
			comAjax.ajax();
		}
		
		function fn_selectBuyListCallback(data) {
			...
			if (total == 0) {
				...
				$.each(data.list, function(key, value) {
					str += "<tr>" +
							"<td>" + value.RNUM + "</td>" +
							"<td class='title'>" +
								"<a href='#this' name='title' class='btn btn-danger'>" + value.REL_TRANS_CGG_CODE + "</a>" +
								"<input type='hidden' id='seq' name='seq' value='"+ value.SEQ + "'>" +
							"</td>" +
							"<td>" + value.SHIP_TOT_TONS + " 톤</td>" +
							"<td>" + value.FS_MET_CODE + "</td>" +
				...
				$("a[name='title']").on("click", function(e) { //제목
					e.preventDefault();
					fn_openBuyDetail($(this));
				});
				
				$("a[name='broker']").on("click", function(e) { //중개인 목록으로
					e.preventDefault();
					fn_buyBrokerList($(this));
				});
			}
		}
		
		function fn_buyBrokerList(obj) {
			var comSubmit = new ComSubmit();
			comSubmit.setUrl("<c:url value='/deal/openBuyBrokerList.do' />");
